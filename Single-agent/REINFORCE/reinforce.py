...
class REINFORCE:
  ...
  def learn(self, total_timesteps):
    ...
    while self.t_so_far < total_timesteps:    
      ...
      lp = self.evaluate(batch_obs, batch_acts)
      policy_gradient = torch.multiply(-lp, batch_rtgs)

      for _ in range(self.n_updates_per_iteration):
        curr_log_probs = self.evaluate(batch_obs, batch_acts)
        policy_gradient = torch.multiply(-curr_log_probs, batch_rtgs)
        pg.append(policy_gradient)
  ...
  def compute_rtgs(self, batch_rews):
    ...
    for ep_rews in reversed(batch_rews):
      ...
      for rew in reversed(ep_rews):
        ...
        discounted_reward = discounted_reward + rew * self.gamma**pw
