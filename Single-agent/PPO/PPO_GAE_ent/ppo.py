...
class PPO:
  ...
  def __init__(self):
    ...
    self.act_dim = 1 #choose a radius 
    ...
    if self.restart == 1:
      ...
      self.actor_optim.load_state_dict(torch.load('./ppo_actor_opt.pth'))
      self.critic.load_state_dict(torch.load('./ppo_critic.pth'))
      self.critic_optim.load_state_dict(torch.load('./ppo_critic_opt.pth'))
  ...
  def learn(self, total_timesteps):
    ...
    while self.t_so_far < total_timesteps:    
      ...
      # This is the loop where we update our network weights for some n epochs
      for _ in range(self.n_updates_per_iteration):
        ...
        # Calculate normal surrogate loss
        surr1 = ratios * batch_advs
        # Calculate surrogate loss with trust region
        surr2 = torch.clamp(ratios, 1 - self.clip, 1 + self.clip) * batch_advs

        # actor and critic losses
        actor_loss_min = (-torch.min(surr1, surr2)).mean()
        actor_loss = actor_loss_min - dist_entropies*self.entropy_beta
        critic_loss = nn.MSELoss()(V, batch_rtgs)
  
        # Calculate gradients and perform backward propagation for actor network
        self.actor_optim.zero_grad()
        actor_loss.backward(retain_graph=True) #Only one backpropagation at a time
  ...
  def rollout(self,obs):
    ...
    while t < self.timesteps_per_batch:
      ...
      for ep_t in range(self.max_timesteps_per_episode):
        ...
        #Reward if |((p_inl-p_0)/(p_33-p_0))-1| is close to (T_outl-T_0)/(T_33-T_0) 
        rew=0
        rew_p=np.abs(obs[1]-1) #pressure
        rew_T=obs[2] #Temperature       
        rew_delta=np.abs(rew_p-rew_T) #This can theoretically be in the range of [0,2], but we want to give a high reward if it is small
        rew=2-rew_delta #Therefore subtract from 2
  ...
  def reset_restart_property_file(self):
    ...
    new_file = open("properties_restart.toml", "w+") 
    for line in lines:
       new_file.write(line)
  ...
  def update_restart_property_file(self,ts):
    ...
    a_file = open("properties_restart.toml", "r") 
    lines = a_file.readlines()
    a_file.close()
