...
class PPO:
  ...
  def __init__(self):
    ...
    # Else init log file if new start
       ...
       rl_log_file.write("self.i_so_far, self.t_so_far, avg_ep_rews_1, avg_ep_rews_2, avg_ep_rews_3, self.ep_rew_best, self.R_1_best, self.R_2_best, self.R_3_best, avg_actor_loss, avg_critic_loss, self.R_1_batch, self.R_1_sum, self.R_1_avg, self.R_2_batch, self.R_2_sum, self.R_2_avg, self.R_3_batch, self.R_3_sum, self.R_3_avg, self.act_lr, self.cri_lr \n")
       rl_log_file.close()

       reward_log_file = open("reward_updated.log", "w") #write to file without line
       reward_log_file.write("self.i_so_far , t , center_1 , radius_1 , center_2, radius_2, center_3, radius_3, p_33, p , |((p_inl-p_0)/(p_33-p_0))-1| , T_33, T , (T_outl-T_0)/(T_33-T_0) , rew_sum, rew \n")
       reward_log_file.close()

       reward_log_file = open("best_reward_updated.log", "w") #write to file without line
       reward_log_file.write("self.i_so_far, t, radius_1, radius_2, radius_3, best_radius_1, best_radius_2, best_radius_3, reward, rewards_1, rewards_2, rewards_3 \n")
    ...
    self.logger = {
           ...
           'critic_losses_3': [],    #losses of critic network in current iteration
      }
  ...
  def learn(self, total_timesteps):
    ...
    if self.restart == 0:
       ...
       R_3_sum_before_restart = np.loadtxt('rl.log', delimiter ="," , usecols =[16], skiprows=1)
       ...
       R_3_best_before_restart = np.loadtxt('rl.log', delimiter ="," , usecols =[7], skiprows=1)
       ...
       self.R_3_best = int(R_3_best_before_restart[iterations_before_restart.shape[0]-1])
       ...
       self.R_3_sum = int(R_3_sum_before_restart[iterations_before_restart.shape[0]-1])

    #Initialize observation array  
    obs_1 = np.zeros((self.obs_dim))
    obs_2 = np.zeros((self.obs_dim))
    obs_3 = np.zeros((self.obs_dim))
    obs_init = np.zeros((self.obs_dim))
    self.reset_restart_property_file()
    # Generate domain with initial bump
    obs_1[0],obs_1[1],obs_1[2] = generate_geometry_start(self.bump_center_1,self.bump_center_2,self.bump_center_3,self.bump_radius_init,self.bump_radius_init,self.bump_radius_init,self.bump_radius_min,self.bump_radius_max)
 
    # Run simulation with initial bump
    os.system("$MPIEXEC $FLAGS_MPI_BATCH /hpcwork/jara0203/RL/MAIA/Solver/src/maia properties_run.toml > maia_command.log")
      
    obs_1[3], obs_1[4], p,T = self.response_from_maia()
    ...
    rewards_3 = np.zeros((5))
    best_radius_1 = np.zeros((5))
    best_radius_2 = np.zeros((5))
    best_radius_3 = np.zeros((5))

    # Overall loop
    while self.t_so_far < total_timesteps:    
    
      # Perform one batch with "time_steps_per_batch" time steps  
      batch_obs_1, batch_obs_2, batch_obs_3, batch_acts_1, batch_acts_2, batch_acts_3, batch_log_probs_1, batch_log_probs_2, batch_log_probs_3, batch_rtgs_1, batch_rtgs_2, batch_rtgs_3, batch_advs_1, batch_advs_2, batch_advs_3, batch_lens = self.rollout(obs_1,obs_2, obs_3, obs_init, rewards_1, rewards_2, rewards_3, best_radius_1, best_radius_2, best_radius_3)
      ...
      for _ in range(self.n_updates_per_iteration):
        ...
        # Calculate V_phi and pi_theta(a_t | s_t)
        V_3, curr_log_probs_3, dist_entropies_3 = self.evaluate(batch_obs_3, batch_acts_3)

        # Calculate ratios
        ratios_3 = torch.exp(curr_log_probs_3 - batch_log_probs_3)

        # Calculate surrogate losses
        surr1_3 = ratios_3 * batch_advs_3
        surr2_3 = torch.clamp(ratios_3, 1 - self.clip, 1 + self.clip) * batch_advs_3

        # actor and critic losses
        actor_loss_min_3 = (-torch.min(surr1_3, surr2_3)).mean()
        actor_loss_3 = actor_loss_min_3 - dist_entropies_3 * self.entr_loss_weight
        critic_loss_3 = nn.MSELoss()(V_3, batch_rtgs_3)        

        # Calculate gradients and perform backward propagation for actor network
        self.actor_optim.zero_grad()
        actor_loss_3.backward(retain_graph=True) #Only one backpropagation at a time
        nn.utils.clip_grad_norm_(self.actor.parameters(), 0.5) #Gradient clipping
        self.actor_optim.step()
        ...
        self.logger['actor_losses_3'].append(actor_loss_3.detach())
        self.logger['critic_losses_3'].append(critic_loss_3.detach())
  ...
  def rollout(self,obs_1,obs_2, obs_3, obs_init, rewards_1, rewards_2, rewards_3, best_radius_1, best_radius_2, best_radius_3):
    ...
    while t < self.timesteps_per_batch:
      ...
      for ep_t in range(self.max_timesteps_per_episode):
        ...
        # Generate new domain
        obs_2[0],obs_2[1],obs_2[2],death_max1,death_min1, death_max2, death_min2, death_max3, death_min3 = generate_geometry_train(self.i_so_far,t,self.bump_center_1,self.bump_center_2,self.bump_center_3,batch_obs_1[-1][0],batch_obs_1[-1][1],batch_obs_1[-1][2],radius_change_1,radius_change_2,radius_change_3,self.bump_radius_min,self.bump_radius_max)
        ...
        #Take response (delta p, delta t)  
        obs_2[3], obs_2[4], p, T = self.response_from_maia()
        os.system("rm Output_SegNo_0_BC_1022.dat")
        os.system("rm Output_SegNo_1_BC_4130.dat")
        
        rew=0
        rew_p=np.abs(obs_2[3]-1) #pressure
        rew_T=obs_2[4] #Temperature       
        rew_max=rew_p+rew_T
        ...
        radius_denorm_3=obs_2[2]*(self.bump_radius_max-self.bump_radius_min)+self.bump_radius_min
        ...
        best_reward_log_file.write(str(self.i_so_far) + " , " + str(t) + " , " + str(radius_denorm_1) + " , " + str(radius_denorm_2) + " , " + str(radius_denorm_3) + " , " + str(best_radius_1) + " , " + str(best_radius_2) + " , " + str(best_radius_3) + " , " + str(rew) + " , " + str(rewards_1) + " , " + str(rewards_2) + " , " + str(rewards_3) + "\n")
        best_reward_log_file.close()

        #Write to reward.log file
        reward_log_file = open("reward_updated.log", "a") #write to file without line
        reward_log_file.write(str(self.i_so_far) + " , " + str(t) + " , " + str(self.bump_center_1) + " , " + str(radius_denorm_1) + " , " + str(self.bump_center_2) + " , " + str(radius_denorm_2) + " , " + str(self.bump_center_3) + " , " + str(radius_denorm_3) + " , " + str(self.p_33) + " , " + str(p) + " , " +str(rew_p) + " , " + str(self.T_33) + " , " + str(T) + " , " + str(rew_T) + " , " + str(rew_max) + " , " + str(rew) + "\n")
        reward_log_file.close()
        ...
        # Generate new domain
        obs_3[0],obs_3[1],obs_3[2],death_max1, death_min1, death_max2, death_min2, death_max3, death_min3 = generate_geometry_train(self.i_so_far,t,self.bump_center_1,self.bump_center_2,self.bump_center_3,batch_obs_2[-1][0],batch_obs_2[-1][1],batch_obs_2[-1][2],radius_change_1,radius_change_2,radius_change_3,self.bump_radius_min,self.bump_radius_max)
        # Run simulation
        os.system("$MPIEXEC $FLAGS_MPI_BATCH /hpcwork/jara0203/RL/MAIA/Solver/src/maia properties_restart.toml > maia_command.log")
        ...
        obs_3[3], obs_3[4], p, T = self.response_from_maia()
        ...
        rew=0
        rew_p=np.abs(obs_3[3]-1) #pressure
        rew_T=obs_3[4] #Temperature       
        rew_max=(rew_p+rew_T)

        #Scale reward
        rew=rew_max*rew_max*rew_max*rew_max*rew_max
        rew=rew/(1.35*1.35*1.35*1.35*1.35)
        radius_denorm_4=obs_1[0]*(self.bump_radius_max-self.bump_radius_min)+self.bump_radius_min
        radius_denorm_5=obs_1[1]*(self.bump_radius_max-self.bump_radius_min)+self.bump_radius_min
        radius_denorm_6=obs_1[2]*(self.bump_radius_max-self.bump_radius_min)+self.bump_radius_min

        # Collect statistics for best ep rew and the corresponding radii
        if rew > self.ep_rew_best:
           self.ep_rew_best=rew
           self.R_1_best = radius_denorm_4
           self.R_2_best = radius_denorm_5
           self.R_3_best = radius_denorm_6
        if death_max2 == True:
           rew = 0
           obs_2[0] = obs_init[0]
           obs_2[1] = obs_init[1]
           ...
           radius_denorm_5 = self.bump_radius_init
           radius_denorm_6 = self.bump_radius_init
           done = True
        ...
        if ep_t < 5:
           rewards_2[ep_t] = rew
           best_radius_2[ep_t] = radius_denorm_5
        if ep_t >= 5:
        ...
        #AGENT 3
        # Collect observation 
        batch_obs_3.append(obs_3)
        last_obs_3 = batch_obs_3[-1]
        #Irint(last_obs_2)
        action_3, log_prob_3 = self.get_action(obs_3,3) # action --> get output from actor network, log_prob --> get the prob distribution of actions
        
        # Collect value
        action_tensor_3 = torch.tensor(action_3, dtype=torch.float)
        V_3 , _ , _ = self.evaluate(obs_3,action_tensor_3)

        radius_change_1=0
        radius_change_2=0
        radius_change_3=action_3*3

        # Generate new domain
        obs_1[0],obs_1[1],obs_1[2],death_max1, death_min1, death_max2, death_min2, death_max3, death_min3 = generate_geometry_train(self.i_so_far,t,self.bump_center_1,self.bump_center_2,self.bump_center_3,batch_obs_3[-1][0],batch_obs_3[-1][1],batch_obs_3[-1][2],radius_change_1,radius_change_2,radius_change_3,self.bump_radius_min,self.bump_radius_max)
        # Run simulation
        os.system("$MPIEXEC $FLAGS_MPI_BATCH /hpcwork/jara0203/RL/MAIA/Solver/src/maia properties_restart.toml > maia_command.log")
        ...
        os.system("rm Output_SegNo_1_BC_4130.dat")
        rew=0
        rew_p=np.abs(obs_1[3]-1) #pressure
        rew_T=obs_1[4] #Temperature       
        rew_max=(rew_p+rew_T)

        #Scale reward
        rew=rew_max*rew_max*rew_max*rew_max*rew_max
        rew=rew/(1.35*1.35*1.35*1.35*1.35)
        radius_denorm_7=obs_1[0]*(self.bump_radius_max-self.bump_radius_min)+self.bump_radius_min
        radius_denorm_8=obs_1[1]*(self.bump_radius_max-self.bump_radius_min)+self.bump_radius_min
        radius_denorm_9=obs_1[2]*(self.bump_radius_max-self.bump_radius_min)+self.bump_radius_min

        # Collect statistics for best ep rew and the corresponding radii
        if rew > self.ep_rew_best:
           self.ep_rew_best=rew
           self.R_1_best = radius_denorm_7
           self.R_2_best = radius_denorm_8
           self.R_3_best = radius_denorm_9
        if death_max3 == True:
           rew = 0
           obs_2[0] = obs_init[0]
           obs_2[1] = obs_init[1]
           ...
           obs_3[3] = obs_init[3]
           obs_3[4] = obs_init[4]
           radius_denorm_7 = self.bump_radius_init
           radius_denorm_8 = self.bump_radius_init
           radius_denorm_9 = self.bump_radius_init
           done = True

        elif death_min3 == True:        
           rew = 0
           obs_2[0] = obs_init[0]
           obs_2[1] = obs_init[1]
           obs_2[2] = obs_init[2]
        ...
        if ep_t < 5:
           rewards_3[ep_t] = rew
           best_radius_3[ep_t] = radius_denorm_9
        if ep_t >= 5:
           idx_3 = np.argmin(rewards_3)
           min_3 = rewards_3[idx_3]
           if rew > min_3:
              rewards_3[idx_3] = rew
           else:
              rewards_3[idx_3] = min_3
        best_reward_log_file = open("best_reward_updated.log", "a") #write to file without line
        best_reward_log_file.write(str(self.i_so_far) + " , " + str(t) + " , " + str(radius_denorm_1) + " , " + str(radius_denorm_2) + " , " + str(radius_denorm_3) + " , " + str(best_radius_1) + " , " + str(best_radius_2) + " , " + str(best_radius_3) + " , " + str(rew) + " , " + str(rewards_1) + " , " + str(rewards_2) + " , " + str(rewards_3) + "\n")
        best_reward_log_file.close()
        ...
        # Collect reward, value, action, and log prob
        ep_rews_3.append(rew)
        ep_vals_3.append(V_3)
        batch_acts_3.append(action_3)
        batch_log_probs_3.append(log_prob_3)
      ...
      batch_rews_3.append(ep_rews_3)
      batch_vals_1.append(ep_vals_1)
      batch_vals_2.append(ep_vals_2)
      batch_vals_3.append(ep_vals_3)
    ...
    batch_log_probs_3 = torch.tensor(batch_log_probs_3, dtype=torch.float)
    ...
    batch_rtgs_3, batch_advs_3 = self.compute_advantage(batch_rews_3,batch_vals_3)
    
    # Log the episodic returns and episodic lengths in this batch.
    self.logger['batch_rews_1'] = batch_rews_1
    self.logger['batch_rews_2'] = batch_rews_2
    self.logger['batch_rews_3'] = batch_rews_3
    self.logger['batch_lens'] = batch_lens

    # Return the batch data
    return batch_obs_1, batch_obs_2, batch_obs_3, batch_acts_1, batch_acts_2, batch_acts_3, batch_log_probs_1, batch_log_probs_2, batch_log_probs_3, batch_rtgs_1, batch_rtgs_2, batch_rtgs_3, batch_advs_1, batch_advs_2, batch_advs_3, batch_lens
  ...
  def _log_summary(self):
    ...
    avg_ep_rews_3 = np.mean([np.sum(ep_rews_3) for ep_rews_3 in self.logger['batch_rews_3']])
    ...
    avg_actor_loss_3 = np.mean([losses.float().mean() for losses in self.logger['actor_losses_3']])
    avg_actor_loss = 0.333333*(avg_actor_loss_1+avg_actor_loss_2+avg_actor_loss_3)
    ...
    avg_critic_loss_3 = np.mean([losses.float().mean() for losses in self.logger['critic_losses_3']])
    avg_critic_loss = 0.333333*(avg_critic_loss_1+avg_critic_loss_2+avg_critic_loss_3)

    # Round decimal places for more aesthetic logging messages
    avg_ep_lens = str(round(avg_ep_lens, 2))
    avg_ep_rews_1 = str(round(avg_ep_rews_1, 2))
    avg_ep_rews_2 = str(round(avg_ep_rews_2, 2))
    avg_ep_rews_3 = str(round(avg_ep_rews_3, 2))
    avg_actor_loss = str(round(avg_actor_loss, 5))
    avg_critic_loss = str(round(avg_critic_loss, 5))

    #Write to log file
    rl_log_file = open("rl.log", "a") #write to file without line
    rl_log_file.write(str(self.i_so_far) + " , " + str(self.t_so_far) + " , " + str(avg_ep_rews_1) + " , " + str(avg_ep_rews_2) + " , " + str(avg_ep_rews_3) + " , " + str(round(self.ep_rew_best, 3)) + " , " + str(round(self.R_1_best,2)) + " , " + str(round(self.R_2_best,2)) + " , " + str(round(self.R_3_best,2)) + " , " + str(avg_actor_loss) + " , " + str(avg_critic_loss) + " , " + str(round(self.R_1_batch,2)) + " , " + str(round(self.R_1_sum,2)) + " , " + str(round(self.R_1_avg,2)) + " , " + str(round(self.R_2_batch,2)) + " , " + str(round(self.R_2_sum,2)) + " , " + str(round(self.R_2_avg,2)) + " , " + str(round(self.R_3_batch,2)) + " , " + str(round(self.R_3_sum,2)) + " , " + str(round(self.R_3_avg,2)) + " , " + str(self.actor_lr) + " , " + str(self.critic_lr) + "\n")
    rl_log_file.close()
    # Reset batch-specific logging data
    self.logger['batch_lens'] = []
    self.logger['batch_rews_1'] = []
    self.logger['batch_rews_2'] = []
    self.logger['batch_rews_3'] = []
    self.logger['actor_losses_1'] = []
    self.logger['actor_losses_2'] = []
    self.logger['actor_losses_3'] = []
    self.logger['critic_losses_1'] = []
    self.logger['critic_losses_2'] = []
    self.logger['critic_losses_3'] = []
