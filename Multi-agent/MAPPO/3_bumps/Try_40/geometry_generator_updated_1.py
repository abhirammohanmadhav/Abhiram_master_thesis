...
def generate_geometry_start(bump_center_1,bump_center_2,bump_radius_1,bump_radius_2,bump_radius_min,bump_radius_max):
    ...
    if ((x.shape[0]!=0) and (y.shape[0]!=0)):
        delta_x1 = np.abs(bump_function_array1[:,0]-x[0]).tolist()
        delta_x2 = np.abs(bump_function_array2[:,0]-x[0]).tolist()
        
        delta_y1 = np.abs(bump_function_array1[:,1]-y[0]).tolist()
        delta_y2 = np.abs(bump_function_array2[:,1]-y[0]).tolist()
