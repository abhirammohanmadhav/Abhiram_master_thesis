...
def generate_geometry_train(nr_iter,nr_steps,bump_center_1,bump_center_2,previous_radius_1,previous_radius_2,radius_change_1,radius_change_2,bump_radius_min,bump_radius_max):
    ...
    if np.size(intersection) != 0:
        x = intersection[0]
        y = intersection[1]
        delta_x1 = np.abs(bump_function_array1[:,0]-x).tolist()
        delta_x2 = np.abs(bump_function_array2[:,0]-x).tolist()
        
        delta_y1 = np.abs(bump_function_array1[:,1]-y).tolist()
        delta_y2 = np.abs(bump_function_array2[:,1]-y).tolist()
        ...
        bfam1 = bump_function_array_mod_1[:idx1,:]
        bfam2 = bump_function_array_mod_2[idx2:,:]
        # bfam11 = bump_function_array_mod_1[:idx1+1,0]
        # bfam12 = bump_function_array_mod_1[:idx1+1,1]
        # bfam21 = bump_function_array_mod_2[idx2:,0]
        # bfam22 = bump_function_array_mod_2[idx2:,1]
        # if bump_radius_1 == bump_radius_2:
        #     b = bfam1[-1:,1]
        #     c = bfam2.[0:,1]
        #     a = (b+c)/2
        ...
        # fig, ax = plt.subplots()
        # ax.plot(bfam1[:,0], bfam1[:,1], 'b-')
        # ax.plot(bfam2[:,0], bfam2[:,1], 'r-')
        # ax.set_xlim(0,200)
        # ax.set_ylim(0,20)
        # # plt.savefig('D:\RWTH\Master_thesis\Mario_MARL\Try_69\Bump 1 and 2.png')
        # plt.show()
        
    elif np.size(intersection) == 0:
        #Write files
        #Inlet
        f = open("Inlet", "w")
        f.write("2"+"\n")
        ...
        #Channel
        f = open("LS_Channel.0", "w")
        f.write(str(bump_function_array_mod_1.shape[0]+bump_function_array_mod_2.shape[0]+5)+"\n")
        f.write("\n")
        f.write(str(end_x+diameter)+" "+"0"+"\n")
        f.write(str(end_x+diameter)+" "+str(diameter)+"\n")
        f.write("-"+str(diameter)+" "+str(diameter)+"\n")
